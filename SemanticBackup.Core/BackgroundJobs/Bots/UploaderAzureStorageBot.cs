    ...
    internal class UploaderAzureStorageBot : IBot
    {
        ...
        public async Task RunAsync()
        {
            ...
            try
            {
                ...
                RSAzureBlobStorageSettings settings = GetValidDeserializedSettings();
                stopwatch.Start();
                //Upload FTP
        ...
        private RSAzureBlobStorageSettings GetValidDeserializedSettings()
        {
            var config = JsonConvert.DeserializeObject<RSAzureBlobStorageSettings>(this._contentDeliveryConfiguration.Configuration);
            if (config == null)
                throw new Exception($"Invalid Configuration String provided Of Type: {nameof(RSAzureBlobStorageSettings)}");
            return config;
        }
