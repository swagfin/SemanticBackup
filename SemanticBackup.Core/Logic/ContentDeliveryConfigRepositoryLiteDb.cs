...
namespace SemanticBackup.Core.Logic
{
    public class ContentDeliveryConfigRepositoryLiteDb : IContentDeliveryConfigRepository
    {
        ...
        {
#if DEBUG
            this._db = new LiteDatabaseAsync(new ConnectionString(System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "backup-delivery-configs.dev.db")) { Password = "12345678", Connection = ConnectionType.Shared });
#else
            this._db = new LiteDatabaseAsync(new ConnectionString(System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "backup-delivery-configs.db")) { Password = "12345678", Connection = ConnectionType.Shared });
#endif
            //Init
